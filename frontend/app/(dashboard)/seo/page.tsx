  ...
  return (
    <div>
      ...
      <div className="flex gap-1 mb-6 border-b border-ink-200 pb-2">
        {(["clusters", "authority", "cannibal"] as Tab[]).map((tabKey) => (
          <button
            key={tabKey}
            onClick={() => setTab(tabKey)}
            className={`text-sm px-4 py-2 rounded-md ${
              tab === tabKey ? "bg-brand-50 text-brand-700 font-medium" : "text-ink-700 hover:bg-ink-50"
            }`}
          >
            {t.seo.tabs[tabKey]}
          </button>
  ...
  return (
    <div>
      <div className="flex justify-between items-start mb-4 flex-wrap gap-2">
        <p className="text-sm text-ink-500 max-w-2xl">{t.seo.clusters.hint}</p>
        <button className="btn-primary text-sm" onClick={build} disabled={building}>
          {building ? t.seo.clusters.working : clusters.length === 0 ? t.seo.clusters.build : t.seo.clusters.rebuild}
        </button>
      </div>
      ...
      {clusters.length === 0 && !building && (
        <div className="card card-pad text-sm text-ink-500">{t.seo.clusters.empty}</div>
      )}
        ...
        {clusters.map((c) => (
          <div key={c.name} className="card card-pad">
            <div className="flex items-start justify-between gap-2 mb-2">
              ...
              <span className="badge badge-cat">{c.size} {t.seo.clusters.keywords}</span>
            </div>
...
function AuthorityTab() {
  ...
  const verdictLabel = (v: string) => v === "strong" ? t.seo.authority.strong : v === "medium" ? t.seo.authority.medium : t.seo.authority.weak;

  return (
    <div>
      <p className="text-sm text-ink-500 mb-4 max-w-2xl">{t.seo.authority.hint}</p>
        ...
        {(data.clusters || []).map((c) => (
          <div key={c.cluster} className="card card-pad">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
              <Stat label={t.seo.authorityStats.keywords} value={c.size.toString()} />
              <Stat label={t.seo.authorityStats.covered} value={`${c.covered}/${c.size} (%${c.coverage_pct.toFixed(0)})`} />
              <Stat label={t.seo.authorityStats.avgPos} value={c.avg_position?.toFixed(1) ?? "—"} />
              <Stat label={t.seo.authorityStats.totalClicks} value={c.total_clicks.toString()} />
            </div>
          </div>
        ))}
      </div>
...
function CannibalTab() {
  ...
  return (
    <div>
      <p className="text-sm text-ink-500 mb-4 max-w-2xl">{t.seo.cannibal.hint}</p>

      {items.length === 0 ? (
        <div className="card card-pad text-sm text-emerald-700">{t.seo.cannibal.empty}</div>
      ) : (
              ...
              <div className="flex items-start justify-between mb-3 flex-wrap gap-2">
                <div>
                  ...
                  <p className="text-xs text-ink-500 mt-0.5">
                    {t.seo.cannibal.pagesCompete(it.page_count)} · {it.total_impressions} {t.seo.cannibal.impressions} · {it.total_clicks} {t.seo.cannibal.clicks}
                  </p>
                </div>
                <span className={`badge ${it.severity === "high" ? "badge-hot" : "bg-amber-50 text-amber-700 border-amber-100"}`}>
                  {it.severity === "high" ? t.seo.cannibal.severityHigh : t.seo.cannibal.severityMedium}
                </span>
              </div>
                ...
                {it.pages.map((p, j) => (
                  <div key={j} className={`flex items-center justify-between p-2 rounded ${j === 0 ? "bg-emerald-50/50 border border-emerald-100" : "bg-white border border-ink-100"}`}>
                    ...
                    <div className="text-xs text-ink-500 tabular-nums shrink-0 ml-2">
                      {t.seo.cannibal.pos} {p.position.toFixed(1)} · {p.clicks} {t.seo.cannibal.clicks}
                    </div>
                  </div>
                ))}
              </div>
